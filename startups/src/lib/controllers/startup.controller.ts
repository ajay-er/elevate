...
router.post('/send-logo', upload.single('logo'),
    async (req: Request, res: Response) => {
        ...
        res.status(200).json({ url:response.secure_url ,message: 'logo added successfully' });
    });

router.post('/send-banner', upload.single('bannerImage'),
    async (req: Request, res: Response) => {
        const image = req?.file;
        const path = image?.path;
        const response = await cloudinary.uploader.upload(path!, {
        ...
        res.status(200).json({url:response.secure_url ,message: 'banner added successfully' });
    });
