    ...
    <tbody>
      <ng-container *ngIf="allSubscriptions.length > 0">
        <tr *ngFor="let item of allSubscriptions" class="bg-white  cursor-pointer bg-opacity-5 border border-white sm:mt-8 rounded-lg sm:p-4 p-2 text-center">
          ...
          <td *ngIf="item.status === 'ACTIVE'" class="bg-green-100 text-green-800 text-xs font-medium text-center  p-1 ">
            {{item.status}}
          </td>
            <td *ngIf="item.status === 'PENDING'"  class="bg-yellow-100 text-yellow-800 text-xs font-medium text-center rounded-lg p-1 sm:p-3 ">
              PENDING
            </td>
            <td *ngIf="item.status === 'CANCELED'" class="bg-red-100 text-red-800 text-xs font-medium text-center  p-1 ">
            {{item.status}}
            </td>
        </tr>
      </ng-container>
